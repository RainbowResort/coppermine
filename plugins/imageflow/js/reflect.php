<?php
 ...
 if (isset($_GET['procent']))
    ...
    if ($hundredwidth < $hundredheight) {
    	$fiftywidth = floor(($hundredheight*$procent)/1.4);
    	$fiftyheight = floor($hundredheight/$hundredwidth*$fiftywidth);
    }
  
  // check if imageflow animation has fixed pixel size. if yes, calculate exact fitting sizes
  if (isset($_GET['fixed'])) {
  	$fixed = $_GET['fixed'];

  	if (($width/$height) >= 1) {
  		$fiftywidth = 0.295 * $fixed;
  	} 
  	if (($width/$height) <= 0.61) {
  		$fiftywidth = 0.22 * $fixed;
  	}
  	if ((($width/$height) < 1) && (($width/$height) > 0.61))
  	{ $fiftywidth = 0.225 * $fixed;
  	}
  	$fiftyheight = (round($hundredheight/$hundredwidth*$fiftywidth));
  	$fiftywidth = (round($fiftywidth));
  }
	...
	*/
	...
	else
	{
		...
		{
			if(!isset( $_GET['jpeg']))  $_GET['jpeg'] = 67;
