...
function cpgif_moveTo(cpgif_x)
{
	...
	for (var cpgif_index = 0; cpgif_index < cpgif_max; cpgif_index++)
	{
		...
		else 
		{
			...
			{
				case false:
					cpgif_image.onmousedown = function() { cpgif_glideTo(this.cpgif_x_pos, this.cpgif_i); };
					break;

				default:
					cpgif_zIndex = cpgif_zIndex + 1;
					cpgif_image.onmousedown = function() { enlarge(this); };
					break;
			}
...
{
  try { document = null; } catch(cpgif_errorli) {}
};
