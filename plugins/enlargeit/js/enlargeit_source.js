...
var enl_transition=1;  // use transition when navigating
...
function enl_init()
{
  if (!enl_firstcall)
  {
    ...
    if (typeof enl_transitionstart == 'undefined') enl_transition = 0;
...
function enl_next(enl_imgid,enl_prvnxt)
{
  if (enl_infront != '')
  {
    ...
    if (enl_nextPic)
    {
      ...
      if (enl_transition) setTimeout('enl_transitionstart("'+enl_imgid+'","'+enl_nextPic.id+'")', 50);
      else
      {
...
// start transition
function enl_transitionstart(enl_oldimgid,enl_newimgid)
{
	alert ("Old image: "+enl_oldimgid+" New image: "+enl_newimgid);
  enl_shrink(enl_oldimgid);
  setTimeout('enl_openthepic("'+enl_newimgid+'")', 50);
}
