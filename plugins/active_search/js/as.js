...
_bsn.AutoSuggest = function (id, param)
{
	...
	// defaults	
	//
	var def = {minchars:1, meth:"get", varname:"input", className:"autosuggest", timeout:2500, delay:500, offsety:-5, shownoresults: true, noresults: "No results!", maxheight: 250, cache: true, type:'Files'};
	for (k in def)
...
_bsn.AutoSuggest.prototype.createList = function(arr)
{
	...
	for (var i=0;i<arr.length;i++)
	{
		...
		var val = arr[i].value;
		var idn = arr[i].id;
		if(this.oP.type == 'Files'){
			var lhref = 'displayimage.php?pos=-';
		}else if(this.oP.type == 'Albums'){
			var lhref = 'thumbnails.php?album=';
		}
		...
		var a 			= _bsn.DOM.cE("a", { href:lhref+idn });
...
function togglesearch(){
	if( document.getElementById("stype").value == "Files"){
		options_xml['type'] = "Albums";
		document.getElementById("stype").value = "Albums";
	}else{
		document.getElementById("stype").value = "Files";
		options_xml['type'] = "Files";
	}
}