...
if (isset($_POST['filelist'])){
	...
	if (!$_POST['directory']) {
    	...
    	$result = $db_query_wrapper($sql);
        if (mysql_num_rows($result)) {
			echo $lang_plugin_mass_import['root_exists']." : " . $_POST['directory'] . "<br />";
			$row = mysql_fetch_row($result);
			$cid = $row[0];
		} else {
			$db_query_wrapper("INSERT INTO {$CONFIG['TABLE_CATEGORIES']} (pos, parent, name) VALUES ('10000', '0', '{$_POST['directory']}')");
...
<form method="POST" action="$scriptname">
	{$lang_plugin_mass_import['subdir_desc']}: <input type="text" name="directory" value=""><br /><br />
	{$lang_plugin_mass_import['sleep_desc']}: <input type="text" name="sleep" value="1000"><br /><br />
	{$lang_plugin_mass_import['autorun_desc']}: <input type="checkbox" name="auto" value="1"><br /><br />
	{$lang_plugin_mass_import['hardlimit_desc']}: <input type="text" name="hardlimit" value="10"><br /><br />
	<input type="submit" name="start" value="{$lang_plugin_mass_import['begin']}">
</form>
