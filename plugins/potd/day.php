...
/**
* Coppermine Photo Gallery 1.4.x day.php
*
...
EOT;
  ...
  $content .= <<< EOT
    <table width="378px" height="410px" border="0" cellpadding="0" cellspacing="0" align="center">
      <tr>
        <td align="center">
          <a href="{$CONFIG['path_to']}displayimage.php?pos=-{$picture['pid']}">$img</a><br />
          <br />
          <big>{$lang_meta_album_names['potd']}</big><br />
          <small>{$lang_meta_album_names['by']}</small>
          <b><a href="{$CONFIG['path_to']}thumbnails.php?album=lastupby&uid={$picture['owner_id']}">{$picture['owner_name']}</a></b><br />
          <br />
          <a href="{$CONFIG['path_to']}thumbnails.php?album=potdarch">{$lang_meta_album_names['potdarch']}</a><br />
        </td>
      </tr>
    </table>
...
if (!$CONFIG['path_to']) { pageheader('Photo of the Day'); }
print $content;
if (!$CONFIG['path_to']) { pagefooter(); }
?>
