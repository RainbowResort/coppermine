	...
	if ($CONFIG['plugin_dst_on'] == '1') {
		if ($CPG_PHP_SELF != 'admin.php') {
			...
			if ($CONFIG['plugin_dst_country'] != '') {
				...
				if ($CONFIG['plugin_dst_on'] == '1') {
					$lang_admin_php['time_offset_detail'] .= sprintf($lang_plugin_dst['dst_will_end_on'], $CONFIG['plugin_dst_datetime']) . ' ' . $lang_plugin_dst['time_will_auto_adjust'];
				} else {
					$lang_admin_php['time_offset_detail'] .= sprintf($lang_plugin_dst['dst_will_start_on'], $CONFIG['plugin_dst_datetime']) . ' ' . $lang_plugin_dst['time_will_auto_adjust'];
				}
			}
		}
	}
