...
require("storage/modules/ftp/config.inc.php");

starttable('100%', $lang_admin_php['storage_servers_manage_ftp_server'], 1);
...
if($superCage->get->testInt('id')) // editing or deleting
{
	...
	{
		$show_form = false;
		$sql = "DELETE FROM {$CONFIG['TABLE_FTP_SERVERS']} WHERE id='".$id."'";
		cpg_db_query($sql);
	...
	{
		$show_form = false;
		$sql = "SELECT * FROM {$CONFIG['TABLE_FTP_SERVERS']} WHERE id='".mysql_escape_string($id)."'";		
		$allowed_post_elements = array("hostname", "username", "password", "prefix_url", "subfolder_path", "ssl_secure_connection", "status");
		...
		if(strlen($sql)>0)
		{
			$sql = "UPDATE {$CONFIG['TABLE_FTP_SERVERS']}".$sql. " WHERE id='".$id."'";
			cpg_db_query($sql);
...
else
{
	if($superCage->post->keyExists('hostname')) // if form is submitted, try to send to mysql
	{
		...
		$form_fields['subfolder_path'] = htmlspecialchars($superCage->post->getRaw('subfolder_path'));
		$form_fields['ssl_secure_connection'] = htmlspecialchars($superCage->post->getRaw('ssl_secure_connection'));
		$form_fields['status'] = htmlspecialchars($superCage->post->getRaw('status'));

		$sql = "INSERT INTO {$CONFIG['TABLE_FTP_SERVERS']} SET quota='536870912000', used='0', free='536870912000', users='0'";
...
?>
				...
				<tr>
					<td class="tableb">
					  <?PHP echo $lang_admin_php['storage_servers_subfolder_path']; ?>
					</td>
					<td class="tableb">
					  <input type="text" name="subfolder_path" size="50" value="<?PHP if(isset($form_fields)) {echo $form_fields['subfolder_path'];} ?>">
					</td>
				</tr>

				<tr>
					<td class="tableb">
					  <?PHP echo $lang_admin_php['storage_servers_ssl_secure_connection']; ?>
					</td>
					<td class="tableb">
					  <select name='ssl_secure_connection'>
						<?PHP
						$ssl_secure_connection_options[0] = "No";
						$ssl_secure_connection_options[1] = "Yes";

						foreach($ssl_secure_connection_options as $value => $name)
						{
							echo "<option value='".$value."'";
							if(isset($form_fields) && $form_fields['ssl_secure_connection']==$value)
								echo " selected";
							echo ">".$name."</option>\n";
						}
