...
if(!isset($CONFIG['storage_ftp_rule']))
	$CONFIG['storage_ftp_rule'] = MIRROR_TO_SOME; //MIRROR_TO_SOME;

if(!isset($CONFIG['storage_ftp_copies_per_file']))
	$CONFIG['storage_ftp_copies_per_file'] = 1; // default is 1 // this is used only if storage_ftp_rule is not equal to MIRROR_TO_ALL
else
	$CONFIG['storage_ftp_copies_per_file'] = (int)$CONFIG['storage_ftp_copies_per_file'];
// TODO: this must exist and must be smaller than.. - make checks

if(!isset($CONFIG['storage_ftp_keep_local_copy']))
	$CONFIG['storage_ftp_keep_local_copy'] = false; // if set to true, copies of all images are stored
// on the local server also

if(!isset($CONFIG['storage_ftp_pic_url_source']))
	$CONFIG['storage_ftp_pic_url_source'] = PIC_URL_SOURCE_RANDOM_SERVER; // where to show the images from
