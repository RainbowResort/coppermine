...
if ($superCage->post->keyExists('submit')) {
    ...
    foreach ($posted_lang_id_array as $posted_lang_id) {
            ###############################################   DB   ##############################################
            $lang_id = $posted_lang_id;
            $lang_english_name = $superCage->post->getEscaped('english_name_'.$posted_lang_id);
            $lang_native_name = $superCage->post->getEscaped('native_name_'.$posted_lang_id);
            $lang_custom_name = $superCage->post->getEscaped('custom_name_'.$posted_lang_id);
            if ($superCage->post->getAlpha('flag_'.$posted_lang_id) != '' && $superCage->post->getAlpha('flag_'.$posted_lang_id) != $lang_langmgr_php['pick_a_flag']) {
                $lang_flag = $superCage->post->getAlpha('flag_'.$posted_lang_id);
            } else {
                $lang_flag = '';
            }
        ...
        if ($superCage->post->getAlpha('new_'.$posted_lang_id) == 'YES') {
            ...
            $query .= ");";*/
            ###########################################    DB    ##########################################
            $cpgdb->query($cpg_db_langmgr_php['add_new_language'], $lang_id, $lang_english_name, $lang_native_name, 
                            $lang_custom_name, $lang_flag, $lang_available, $lang_enable, $lang_complete);
            ##########################################################################################
        } else {
            ...
            ###########################################    DB    ##########################################
            $cpgdb->query($cpg_db_langmgr_php['edit_language'], $lang_english_name, $lang_native_name, $lang_custom_name, 
                            $lang_flag, $lang_available, $lang_enable, $lang_complete, $lang_id);
            ##########################################################################################
        }
