...
function display_group_list()
{
    ...
    global $lang_groupmgr_php, $lang_byte_units, $lang_common, $cpg_db_groupmgr_php;
	####################### DB #########################	
...
/*    $result = cpg_db_query("SELECT * FROM {$CONFIG['TABLE_USERGROUPS']} WHERE 1 ORDER BY group_id");
    ...
    }	*///print($cpg_db_groupmgr_php['insert_administrators']);exit;
################################  DB  ##################################
	$cpgdb->query($cpg_db_groupmgr_php['display_group_list']);
	$rowset = $cpgdb->fetchRowSet();
	if(!count($rowset)) {
		$cpgdb->query($cpg_db_groupmgr_php['insert_administrators']);
		$cpgdb->query($cpg_db_groupmgr_php['insert_registered']);
		$cpgdb->query($cpg_db_groupmgr_php['insert_anonymous']);
		$cpgdb->query($cpg_db_groupmgr_php['insert_banned']);
		cpg_die(CRITICAL_ERROR, $lang_groupmgr_php['error_group_empty'], __FILE__, __LINE__);
	}
#####################################################################	
...
//    while ($group = mysql_fetch_array($result)) {
    foreach ($rowset as $group) {	##############		cpgdb_AL
...
if ($superCage->post->keyExists('del_sel') && $superCage->post->keyExists('delete_group')) {
    ...
    if (is_array($delete_group_arr)) {
        foreach($delete_group_arr as $group_id) {
			...
			####################  DB  #####################
			$cpgdb->query($cpg_db_groupmgr_php['delete_usergroup'], (int)$group_id);
			$cpgdb->query($cpg_db_groupmgr_php['set_default_usergroup'], (int)$group_id);
			############################################
        }
    }
} elseif ($superCage->post->keyExists('new_group')) {
    //cpg_db_query("INSERT INTO {$CONFIG['TABLE_USERGROUPS']} (group_name) VALUES ('')");
	$cpgdb->query($cpg_db_groupmgr_php['new_blank_usergroup']);
