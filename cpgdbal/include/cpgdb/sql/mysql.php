...
if (defined('ADMIN_PHP')) {
    ...
    //'get_user_passwords'    => "SELECT user_id, user_password FROM {$CONFIG['TABLE_USERS']}",
    //'encrypt_passwords'     => "UPDATE {$CONFIG['TABLE_USERS']} set user_password='%1\$s' WHERE user_id = '%2\$s';"
    );
}
...
if (defined('CALENDAR_PHP')) {
    ...
    'get_date_link'     => "SELECT COUNT(pid) AS count FROM {$CONFIG['TABLE_PICTURES']} AS p WHERE approved = 'YES' ".
...
if (defined('CATMGR_PHP')) {
    ...
    'getalpha_move'             => "UPDATE {$CONFIG['TABLE_CATEGORIES']} SET pos='%1\$s', lft=0 WHERE cid = '%2\$s' LIMIT 1",
    //'update_categories_04'      => "UPDATE {$CONFIG['TABLE_CATEGORIES']} SET pos='$pos2' WHERE cid = '$cid2' LIMIT 1",
    'getalpha_setparent'        => "UPDATE {$CONFIG['TABLE_CATEGORIES']} SET parent='%1\$s', pos='-1', lft=0 WHERE cid = '%2\$s' LIMIT 1",
    ...
    'updatecat_no_parent'       => "UPDATE {$CONFIG['TABLE_CATEGORIES']} SET parent='%1\$s', name='%2\$s', description='%3\$s', ".
                                   "thumb='%4\$s', lft=0 WHERE cid = '%5\$s' LIMIT 1",
    'updatecat_parent'          => "UPDATE {$CONFIG['TABLE_CATEGORIES']} SET name='%1\$s', description='%2\$s', thumb='%3\$s', lft=0 WHERE cid = '%4\$s' LIMIT 1",
...
if (defined('DISPLAYIMAGE_PHP')) {
    ...
    'get_current_pic_data'  => "SELECT aid FROM {$CONFIG['TABLE_PICTURES']} AS p WHERE pid='%1\$s' %2\$s LIMIT 1",
...
if (defined('EDITPICS_PHP')) {
    ...
    'process_data_del_pictures'         => "DELETE FROM {$CONFIG['TABLE_PICTURES']} WHERE pid='%1\$s' LIMIT 1",
    ...
    'get_public_albums'                 => "SELECT alb.aid AS aid, CONCAT_WS('', '(', cat.name, ') ', alb.title) AS title ".
...
if (defined('INDEX_PHP')) {
    ...
    'alb_stats_kwrd'                => "SELECT a.aid, COUNT( p.pid ) AS pic_count, MAX( p.pid ) AS last_pid, MAX( p.ctime ) ".
                                       ...
                                       "WHERE a.category = %1\$s %2\$s GROUP BY a.aid %3\$s ",
...
if (defined('PLUGINMGR_PHP') || defined('CORE_PLUGIN')) {
    ...
    'get_plugin'            => "SELECT * FROM ".$CONFIG['TABLE_PLUGINS']." WHERE plugin_id=%1\$s LIMIT 1;",
...
if (defined('RATEPIC_PHP')) {
    ...
    'get_pic_info'          => "SELECT a.votes AS votes_allowed, p.votes AS votes, pic_rating, owner_id ".
                               "FROM {$CONFIG['TABLE_PICTURES']} AS p, {$CONFIG['TABLE_ALBUMS']} AS a ".
                               "WHERE p.aid = a.aid AND pid = '%1\$s' LIMIT 1",
    ...
    'add_vote_stats'        => "INSERT INTO {$CONFIG['TABLE_VOTE_STATS']} SET pid = %1\$s, rating = %2\$s, Ip  = '%3\$s', ".
                               "sdate = '%4\$s', referer = '%5\$s', browser = '%6\$s', os = '%7\$s', uid = '%8\$s'"
    );
}
...
if (defined('REGISTER_PHP')) {
    ...
    'add_user'              => "INSERT INTO {$CONFIG['TABLE_USERS']} (user_regdate, user_active, user_actkey, ".
                               ...
                               "user_profile4, user_profile5, user_profile6, user_language) VALUES (NOW(), '%1\$s', '%2\$s', ".
                               "'%3\$s', '%4\$s', '%5\$s', '%6\$s', '%7\$s', '%8\$s', '%9\$s', '%10\$s', '%11\$s', '%12\$s')",
    'add_user_album'        => "INSERT INTO {$CONFIG['TABLE_ALBUMS']} (`title`, `category`) VALUES ('%1\$s', %2\$s)",
    'get_user_data'         => "SELECT user_active, user_email, user_name, user_password FROM {$CONFIG['TABLE_USERS']} ".
                               "WHERE user_actkey = '%1\$s'  LIMIT 1",
    'set_user_active'       => "UPDATE {$CONFIG['TABLE_USERS']} SET user_active = 'YES' WHERE user_actkey = '%1\$s' LIMIT 1"
    );
}
...
if (defined('REPORT_FILE_PHP')) {
    ...
    'get_comment_info'  => "SELECT msg_id, msg_author, msg_body, UNIX_TIMESTAMP(msg_date) AS msg_date, ".
...
if (defined('STAT_DETAILS_PHP')) {
    ...
    'get_pic_stat_details'  => "SELECT %1\$s.* %2\$s FROM %3\$s %4\$s WHERE %5\$s  ".
                               "ORDER BY %6\$s %7\$s LIMIT %8\$s, %9\$s"
    );
}
...
if (defined('UPDATE_PHP')) {
    ...
    'get_config_value'      => "SELECT value FROM ".$CONFIG['TABLE_PREFIX']."config WHERE name='%1\$s' LIMIT 1",
...
if (defined('UPLOAD_PHP') || defined('DB_INPUT_PHP') || defined('ADMIN_PHP')) {
    ...
    'get_cat_ancestry'              => "SELECT cid, parent, name FROM " . $CONFIG['TABLE_CATEGORIES'] . " WHERE 1",
    'get_all_tempdata_id'           => "SELECT unique_ID FROM {$CONFIG['TABLE_TEMPDATA']}",
    'add_tempdata'                  => "INSERT INTO {$CONFIG['TABLE_TEMPDATA']} VALUES ('%1\$s', '%2\$s', '%3\$s')",
    'update_record_tempdata'        => "UPDATE {$CONFIG['TABLE_TEMPDATA']} SET encoded_string = '%1\$s' ".
                                       "WHERE unique_ID = '%2\$s'",
    'delete_record_tempdata'        => "DELETE FROM {$CONFIG['TABLE_TEMPDATA']} WHERE unique_ID = '%1\$s'",
    'retrieve_record_tempdata'      => "SELECT encoded_string FROM {$CONFIG['TABLE_TEMPDATA']} WHERE unique_ID = '%1\$s'",
...
$cpg_db_phpbb2018_inc = array(
    ...
    'get_user_usergroups'   => "SELECT u.%1\$s AS user_id, MIN(ug.%2\$s) AS user_group, %3\$s AS user_name, %4\$s AS user_email, ".
                               ...
                               "WHERE u.%1\$s > 0 %10\$s   GROUP BY ug.%1\$s %11\$s   LIMIT %12\$s, %13\$s;",
    'get_user_pic_data'     => "SELECT owner_id, COUNT(pid) AS pic_count, ROUND(SUM(total_filesize)/1024) AS disk_usage ".
                               "FROM %1\$s WHERE owner_id IN (%2\$s) GROUP BY owner_id" 
);
...
$cpg_db_punbb12_inc = array(
    ...
    'get_user_usergroups'   => "SELECT u.%1\$s AS user_id, u.%2\$s AS user_group, %3\$s AS user_name, %4\$s AS user_email, ".
                               "%5\$s AS user_regdate, %6\$s AS user_lastvisit, '' AS user_active, 0 AS pic_count ".
                               "FROM %7\$s AS u WHERE u.%1\$s > 1 %8\$s %9\$s LIMIT %10\$s, %11\$s",
    'get_user_pic_data'     => "SELECT owner_id, COUNT(pid) AS pic_count, ROUND(SUM(total_filesize)/1024) AS disk_usage ".
                               "FROM %1\$s WHERE owner_id IN (%2\$s) GROUP BY owner_id"
);
...
$cpg_db_udb_base_inc = array(
    ...
    'admin_alb_cannot_join_tbl'     => "SELECT aid, ".
                                       "IF(category > %1\$s, CONCAT('* ', title), CONCAT(' ', title)) ".
                                       "AS title ".
                                       "FROM {$CONFIG['TABLE_ALBUMS']} ORDER BY title",
...
$cpg_db_xmb_inc = array(
    ...
    'get_user'          => "SELECT %1\$s AS user_id, %2\$s AS user_name, %3\$s AS user_email, %4\$s AS user_regdate, ".
                           ...
                           "LEFT JOIN %9\$s AS p ON p.owner_id = u.%1\$s %10\$s".
                           "GROUP BY user_id ORDER BY %11\$s LIMIT %12\$s, %13\$s;"
);
...
$cpg_db_xoops_inc = array(
    'get_users' => "SELECT u.%1\$s AS user_id, %2\$s AS user_name, %3\$s AS user_email, %4\$s AS user_regdate, ".
...
$cpg_db_functions_inc = array(
    ...
    'get_rgt_lft_depth'             => "SELECT rgt, lft, depth FROM {$CONFIG['TABLE_CATEGORIES']} WHERE cid = %1\$s LIMIT 1",
    ...
    'get_pic_data_lastcom'          => "SELECT %1\$s FROM {$CONFIG['TABLE_COMMENTS']} AS c ".
                                       ...
                                       "AND r.approved = 'YES' AND c.approval = 'YES' ORDER by msg_id DESC %3\$s",
    'count_get_pic_data_lastcomby'  => "SELECT COUNT(*) AS count FROM {$CONFIG['TABLE_COMMENTS']} AS c ".
                                       ...
                                       "AND r.approved = 'YES' AND c.approval = 'YES' ",
    'get_pic_data_lastcomby'        => "SELECT %1\$s FROM {$CONFIG['TABLE_COMMENTS']} AS c ".
                                       "INNER JOIN {$CONFIG['TABLE_PICTURES']} AS r ON r.pid = c.pid ".
                                       "INNER JOIN {$CONFIG['TABLE_ALBUMS']} AS a ON a.aid = r.aid %2\$s ". 
    ...
    'get_pic_data_lastup'           => "SELECT %1\$s FROM {$CONFIG['TABLE_PICTURES']} AS p ".
                                       "INNER JOIN {$CONFIG['TABLE_ALBUMS']} AS r ON r.aid = p.aid %2\$s AND approved = 'YES' ".
                                       "ORDER BY p.pid DESC %3\$s",
    ...
    'get_pic_data_lastupby'         => "SELECT %1\$s FROM {$CONFIG['TABLE_PICTURES']} AS p ".
                                       "INNER JOIN {$CONFIG['TABLE_ALBUMS']} AS r ON r.aid = p.aid ".
                                       "%2\$s AND p.owner_id = '%3\$s' AND approved = 'YES' ORDER BY pid DESC %4\$s",
    ...
    'get_pic_data_topn'             => "SELECT %1\$s FROM {$CONFIG['TABLE_PICTURES']} AS p ".
                                       "INNER JOIN {$CONFIG['TABLE_ALBUMS']} AS r ON r.aid = p.aid %2\$s".
                                       "AND approved = 'YES' AND hits > 0 ORDER BY hits DESC, pid  %3\$s",
    'count_get_pic_data_toprated'   => "SELECT COUNT(*) AS count FROM {$CONFIG['TABLE_PICTURES']} AS p ".
                                       "INNER JOIN {$CONFIG['TABLE_ALBUMS']} AS r ON r.aid = p.aid %1\$s ".
                                       "AND approved = 'YES' AND p.votes > '%2\$s'",
    'get_pic_data_toprated'         => "SELECT %1\$s FROM {$CONFIG['TABLE_PICTURES']} AS p ".
                                       "INNER JOIN {$CONFIG['TABLE_ALBUMS']} AS r ON r.aid = p.aid %2\$s ".
                                       "AND approved = 'YES' AND p.votes > '%3\$s' ORDER BY pic_rating DESC, p.votes DESC, pid DESC %4\$s",
    ...
    'get_pic_data_lasthits'         => "SELECT %1\$s FROM {$CONFIG['TABLE_PICTURES']} AS p ".
                                       "INNER JOIN {$CONFIG['TABLE_ALBUMS']} AS r ON r.aid = p.aid %2\$s AND approved = 'YES' ".
                                       "AND hits > 0 ORDER BY mtime DESC %3\$s ",
    'count_get_pic_data_random'     => "SELECT COUNT(*) AS count FROM {$CONFIG['TABLE_PICTURES']} AS p ".
                                       "INNER JOIN {$CONFIG['TABLE_ALBUMS']} AS r ON r.aid = p.aid %1\$s AND approved = 'YES'",
    'get_random_pid'                => "SELECT pid FROM {$CONFIG['TABLE_PICTURES']} AS p ".
                                       "INNER JOIN {$CONFIG['TABLE_ALBUMS']} AS r ON r.aid = p.aid %1\$s AND approved = 'YES' ".
                                       "ORDER BY RAND() %2\$s ",
    ...
    'get_pic_data_lastalb'          => "SELECT *, r.title AS title, r.aid AS aid FROM {$CONFIG['TABLE_PICTURES']} AS p ".
                                       "INNER JOIN {$CONFIG['TABLE_ALBUMS']} AS r ON r.aid = p.aid %1\$s AND approved = 'YES' ".
                                       "GROUP BY p.aid ORDER BY p.ctime DESC %2\$s ",
    'count_get_pic_data_favpics'    => "SELECT COUNT(*) AS count FROM {$CONFIG['TABLE_PICTURES']} AS p ".
                                       "INNER JOIN {$CONFIG['TABLE_ALBUMS']} AS r ON r.aid = p.aid %1\$s ".
                                       "AND approved = 'YES' AND pid IN (%2\$s) ",
    'get_pic_data_favpics'          => "SELECT %1\$s FROM {$CONFIG['TABLE_PICTURES']} AS p ".
                                       "INNER JOIN {$CONFIG['TABLE_ALBUMS']} AS r ON r.aid = p.aid %2\$s AND approved = 'YES' ".
                                       "AND pid IN (%3\$s) %4\$s",
    /*'count_get_pic_data_datebrowse'    => "SELECT COUNT(pid) AS count FROM {$CONFIG['TABLE_PICTURES']} WHERE approved = 'YES' ".
                                       "AND substring(from_unixtime(ctime),1,10) = '%1\$s' %2\$s",*/
    'count_get_pic_data_datebrowse' => "SELECT COUNT(*) AS count FROM {$CONFIG['TABLE_PICTURES']} AS p ".
                                       "INNER JOIN {$CONFIG['TABLE_ALBUMS']} AS r ON r.aid = p.aid %1\$s AND approved = 'YES' ".
                                       "AND substring(from_unixtime(ctime),1,10) = '%2\$s'",
    ...
    'get_pic_data_datebrowse'       => "SELECT %1\$s FROM {$CONFIG['TABLE_PICTURES']} AS p ".
                                       "INNER JOIN {$CONFIG['TABLE_ALBUMS']} AS r ON r.aid = p.aid %2\$s ".
                                       "AND approved = 'YES' AND substring(from_unixtime(ctime),1,10) = '%3\$s' %4\$s",
    ...
    'chk_langtbl_exists'            => "SHOW TABLES LIKE '{$CONFIG['TABLE_LANGUAGE']}'",
...
$cpg_db_search_inc = array(
    ...
    'final_search_string'       => "SELECT * FROM {$CONFIG['TABLE_PICTURES']} AS p WHERE %1\$s ORDER BY %2\$s %3\$s"
);
...
if (defined('CORE_PLUGIN')) $cpg_db_usergal_alphatabs = array(
    'list_users_can_join_tables'    => "SELECT %1\$s AS user_id, {$f['username']} AS user_name, ".
                                        ...
                                        "INNER JOIN {$CONFIG['TABLE_PICTURES']} AS p ON p.aid = a.aid ".
                                        "WHERE ((isnull(approved) OR approved='YES') AND category > %3\$s) %4\$s ".
                                        //if ($l = $getLetter) $sql .= "AND {$f[\'username\']} LIKE \'$l%\' ";
                                        "%5\$s GROUP BY category  ORDER BY category LIMIT %6\$s, %7\$s ",
    'user_have_alb_pics'            => "SELECT category - 10000 AS user_id  FROM {$CONFIG['TABLE_ALBUMS']} AS a ".
                                        "INNER JOIN {$CONFIG['TABLE_PICTURES']} AS p ON p.aid = a.aid ".
                                        "WHERE ((isnull(approved) OR approved='YES') ".
                                        "AND category > %1\$s) %2\$s GROUP BY category  LIMIT %3\$s, %4\$s",
...
$cpg_db_sample_theme_php = array(
    ...
    'get_pic_ratings'           => "SELECT * FROM {$CONFIG['TABLE_VOTES']} WHERE pic_id=%1\$s  ".
                                   "AND user_md5_id='%2\$s'",
    'get_comments'              => "SELECT msg_id, msg_author, msg_body, UNIX_TIMESTAMP(msg_date) AS msg_date, ".
