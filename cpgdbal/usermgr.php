...
function list_group_alb_access($group_id) {  //shows a list of albums a specific group can see. Categories are listed with albums for clarity
    global $CONFIG, $group_id, $aid, $cpg_db_usermgr_php;
	####################### DB #########################	
		$cpgdb =& cpgDB::getInstance();
		$cpgdb->connect_to_existing($CONFIG['LINK_ID']);
	##################################################			
	...
	#####################  DB  #########################
	$cpgdb->query($cpg_db_usermgr_php['list_group_alb_access'], $group_id);
	$albs = $cpgdb->fetchRowSet();
	$cpgdb->free();
	#################################################
...
#########################  DB  #############################
	$cpgdb->query($cpg_db_usermgr_php['list_groups_alb_access']);
	$groups = $cpgdb->fetchRowSet();
	$cpgdb->free();
##########################################################
    echo "
...
EOT;
    ...
    // query total space used
    $result = $cpgdb->query($cpg_db_usermgr_php['total_space_used']);
    $tempSpaceCount = $cpgdb->fetchRow();
    $totalSpaceCount = $tempSpaceCount['sum'];
    $totalSpaceCount_fmt = cpg_float2decimal($totalSpaceCount);
    $cpgdb->free();
    unset($tempSpaceCount);
    
    // query total number of comments posted
    $result = $cpgdb->query($cpg_db_usermgr_php['total_comments_posted']);
    $tempCommentCount = $cpgdb->fetchRow();
    $totalCommentCount = $tempCommentCount['count'];
    $totalCommentCount_fmt = cpg_float2decimal($totalCommentCount);
    $cpgdb->free();
    unset($tempCommentCount);
    #############################################################
    ...
    foreach ($users as $user) {
        ...
        mysql_free_result($result); */
        ###################################      DB       ##################################
        $approval = ($lim_user == 0) ? "": " AND approval = 'YES'"; 
        $cpgdb->query($cpg_db_usermgr_php['get_comment_count'], $user['user_id'], $approval);
        $commentCount = $cpgdb->fetchRow();
        $user['comment_count'] = $commentCount['count'];
        $cpgdb->free();
        ############################################################################
...
EOT;
       /*$sql = "SELECT group_id, group_name FROM {$CONFIG['TABLE_USERGROUPS']} ORDER BY group_name";
        ...
        mysql_free_result($result);*/	
	#############################  DB  #################################
		 $cpgdb->query($cpg_db_usermgr_php['list_users']);
		$group_list = $cpgdb->fetchRowSet();
		$cpgdb->free();
	#################################################################
...
function edit_user($user_id)
{
    ...
    mysql_free_result($result);*/	
#############################  DB  ################################
	$cpgdb->query($cpg_db_usermgr_php['get_edit_user'], $user_id);
	$rowset = $cpgdb->fetchRowSet();
	if(!count($rowset))	cpg_die(CRITICAL_ERROR, $lang_usermgr_php['err_unknown_user'], __FILE__, __LINE__);
	$user_data = $rowset[0];//$cpgdb->Record;
	$cpgdb->free();
################################################################	
...
function update_user($user_id)
{
    ...
    mysql_free_result($result);	*/
############################################
	$cpgdb->query($cpg_db_usermgr_php['get_update_user'],$user_name, $user_id);
	$rowset = $cpgdb->fetchRowSet();
	if(count($rowset)){
        cpg_die(ERROR, $lang_register_php['err_user_exists'], __FILE__, __LINE__);
        return false;
...
###########################  DB  ##################################
	...
	$cpgdb->query($cpg_db_usermgr_php['update_user'], $user_name, $user_email, $user_active, $user_group, $profile1, $profile2, $profile3, $profile4, $profile5, $profile6, $user_group_list, $password_update, $user_id);
################################################################	
...
switch ($op) {
    ...
    case 'new_user' :
        //$cpg_udb->edit_users();print($cpg_db_usermgr_php['switch_new_user']);exit;
        /*cpg_db_query("INSERT INTO {$CONFIG['TABLE_USERS']}(user_regdate, user_active, user_profile6) VALUES (NOW(), 'YES', '')");
        $user_id = mysql_insert_id();*/
	#####################################################
		$cpgdb->query($cpg_db_usermgr_php['switch_new_user']);
		$user_id = $cpgdb->insertId();
	#####################################################
        pageheader($lang_usermgr_php['title']);
        edit_user($user_id);
        pagefooter();
	...
	#########################  DB  ############################
		$cpgdb->query($cpg_db_usermgr_php['switch_group_album_access'], $group_id);
		$rowset = $cpgdb->fetchRowSet();
		$group = $rowset[0];
	########################################################

        //if (!mysql_num_rows($result)) {
		if (!count($rowset)) {	##########################	cpgdb_AL
