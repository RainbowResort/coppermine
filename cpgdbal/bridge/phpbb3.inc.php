...
class cpg_udb extends core_udb {
	...
	function session_extraction()
	{
		...
		if ($superCage->cookie->keyExists($this->cookie_name . '_sid')) {
			...
			####################################################      DB     ###################################################
			$this->cpgudb->query($cpg_db_phpbb_inc['session_extraction'], $this->sessionstable, $this->usertable, $this->session_id);
	...
	function get_users($options = array())
    {
		...
		global $cpg_db_phpbb3_inc;
		$cpgdb =& cpgDB::getInstance();
		$cpgdb->conect_to_existing($CONFIG['LINK_ID']);
		##########################################
		...
		#################################       DB      ################################
		$cpgdb->query($cpg_db_phpbb3_inc['get_cpg_usergroups'], $C['TABLE_USERGROUPS']);
		...
		##################################      DB      ################################
		$this->cpgudb->query($cpg_db_phpbb3_inc['get_udb_group_id'], $this->groupstable);
		$udb_groups = array();
		...
		###########################################################      DB      #######################################################
		$this->cpgudb->query($cpg_db_phpbb3_inc['get_users'], $f['user_id'], $f['usertbl_group_id'], $f['username'], $f['email'], $f['regdate'], 
							$f['lastvisit'], $this->usertable,  $options['search'], $sort, $options['lower_limit'], $options['users_per_page']);
		...
		foreach ($rowset as $user) {	#####	cpgdbAL
		...
		######################################        DB      ####################################
		$cpgdb->query($cpg_db_phpbb3_inc['get_pic_owner'], $C['TABLE_PICTURES'], $user_list_string);
