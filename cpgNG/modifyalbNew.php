...
$data = array($lang_modifyalb_php['general_settings'],
    ...
    array($lang_modifyalb_php['alb_parent'], 'parent', 3),
    array($captionLabel, 'description', 4),
    array($lang_modifyalb_php['alb_keyword'], 'keyword', 0),
    array($lang_modifyalb_php['alb_thumb'], 'thumb', 5),
    $lang_modifyalb_php['alb_perm'],
    array($lang_modifyalb_php['can_view'], 'visibility', 6),
    array($lang_modifyalb_php['alb_password'], 'alb_password', 7),
    array($lang_modifyalb_php['alb_password_hint'], 'alb_password_hint', 8),
...
function get_subalb_data($parent, $ident = '')
{
    global $CONFIG;
    $subalbums=array();
    $result = cpg_db_query("SELECT aid, title, parent FROM {$CONFIG['TABLE_ALBUMS']} WHERE parent = '$parent' ORDER BY title");
    if (mysql_num_rows($result) > 0) {
        $rowset = cpg_db_fetch_rowset($result);
        foreach ($rowset as $subalb) {
             $subalbums[]=array($subalb['aid'], $ident . $subalb['title'], $subalb['parent']);
             $subalbums = array_merge($subalbums,get_subalb_data($subalb['aid'], $ident . '&nbsp;&nbsp;&nbsp;'));
        }
    }
    return $subalbums;
}
...
function form_album($text, $name)
{
    global $ALBUM_DATA, $ALB_LIST, $USER_DATA, $lang_modifyalb_php;
...
EOT;
    ...
    $display_list[]=array(0,$lang_modifyalb_php['no_parent'],0);
    $display_list=array_merge($display_list,$ALB_LIST);
    echo <<<EOT
        <tr>
            <td class="tableb">
                        $text
        ...
        <td class="tableb" valign="top">
                        <select name="$name" class="listbox">
EOT;
    foreach($display_list as $alb_parent) {
        echo '                                <option value="' . $alb_parent[0] . '"' . ($alb_parent[0] == $ALBUM_DATA['parent'] ? ' selected': '') . ">" . $alb_parent[1] . "</option>\n";
    }
    echo <<<EOT
                        </select>
                </td>
        ...
        if ((is_array($element))) {
            switch ($element[2]) {
                ...
                case 2 :
                    ...
                    break;
                case 3 :
                    form_album($element[0], $element[1]);
                    break;
                case 4 :
...
function alb_list_data()
{
    global $CONFIG, $album, $cpg_udb, $ALB_LIST; //, $PHP_SELF;
    $ALB_LIST = array();
    if (GALLERY_ADMIN_MODE) {
        $result = cpg_db_query("SELECT aid, parent, title FROM {$CONFIG['TABLE_ALBUMS']} WHERE parent='0' AND category < '" . FIRST_USER_CAT . "' ORDER BY title");
        $rowset = cpg_db_fetch_rowset($result);
        mysql_free_result($result);
        if (defined('UDB_INTEGRATION')) {
            $sql = $cpg_udb->get_admin_album_list();
        } else {
            $sql = "SELECT aid, parent, CONCAT('(', user_name, ') ', title) AS title " . "FROM {$CONFIG['TABLE_ALBUMS']} AS a " . "INNER JOIN {$CONFIG['TABLE_USERS']} AS u ON category = (" . FIRST_USER_CAT . " + user_id) where parent='0'" . "ORDER BY title";
        }
        $result = cpg_db_query($sql);
        while ($row = mysql_fetch_array($result)) $rowset[] = $row;
        mysql_free_result($result);
    } else {
        $result = cpg_db_query("SELECT aid, parent, title FROM {$CONFIG['TABLE_ALBUMS']} WHERE parent='0' AND category = '" . (FIRST_USER_CAT + USER_ID) . "' ORDER BY title");
        $rowset = cpg_db_fetch_rowset($result);
        mysql_free_result($result);
    }
    foreach ($rowset as $row) { //rowset is all the top level albs that the user is allowed to see.
        $ALB_LIST[]=array($row['aid'], $row['title'], $row['parent']);
        $ALB_LIST=array_merge($ALB_LIST,get_subalb_data($row['aid'],'&nbsp;&nbsp;&nbsp;'));
    }
}

function alb_list_box()
{
    global $ALB_LIST, $album;
    ...
    if (count($ALB_LIST)) {
        ...
        foreach ($ALB_LIST as $row) {
            $selected = ($row[0] == $album) ? "SELECTED" : "";
            $lb .= "        <option value=\"" . $row[0] . "\" $selected>" . $row[1] . "</option>\n";
        }
        $lb .= "</select>\n";
        return $lb;
    }
...
//////////// main code start ///////////////////
alb_list_data(); //populate the global $ALB_LIST
