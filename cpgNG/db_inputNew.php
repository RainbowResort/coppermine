...
function get_subalb_list($parent)
{
    global $CONFIG, $SUBALB_LIST, $lang_errors;
    $result = cpg_db_query("SELECT aid, parent FROM {$CONFIG['TABLE_ALBUMS']} WHERE parent = '$parent' ORDER BY title");
    if (mysql_num_rows($result) > 0) {
        $rowset = cpg_db_fetch_rowset($result);
        foreach ($rowset as $subalb) {
            if (in_array($subalb['aid'],$SUBALB_LIST)) {
                cpg_die(CRITICAL_ERROR, $lang_errors['album_loop'], __FILE__, __LINE__);
            }
            $SUBALB_LIST[] = $subalb['aid'];
            get_subalb_list($subalb['aid']);
        }
    }
}
...
{
    global $CONFIG, $ALB_LIST, $lang_bad_words, $queries;
...
switch ($event) {
    ...
    case 'album_update':
        ...
        if (GALLERY_ADMIN_MODE) {
            $result = cpg_db_query("SELECT aid, parent, category FROM {$CONFIG['TABLE_ALBUMS']} WHERE aid = $aid");//need to find out if the parent or category changed
            if (mysql_num_rows($result) > 0) {
            ...
            if ( $oldParent!= $parent || $row['category'] != $category) { // then set all of the sub albums to the same category as the parent.
               ...
               if (mysql_num_rows($result) > 0) {
                    ...
                    $parentParent=$row['parent'];  //used for preventing album loops
                    $parentAid=$row['aid']; //used for preventing album loops
               } //need else error here
               if ( $oldParent!= $parent) {
                 $category = $parentCategory; //if they chose a new parent, its category overrides any category they just selected.
               } else {
                 $parent=''; //if they've just changed categories we need to unset any parent so we can change all the children to the same category.
               }
            ...
            $query = "UPDATE {$CONFIG['TABLE_ALBUMS']} SET title='$title', parent='$parent', description='$description', category='$category', thumb='$thumb', uploads='$uploads', comments='$comments', votes='$votes', visibility='$visibility', alb_password='$password', alb_password_hint='$password_hint', keyword='$keyword' WHERE aid='$aid' LIMIT 1";
        } else {
            $category = FIRST_USER_CAT + USER_ID;
            $query = "UPDATE {$CONFIG['TABLE_ALBUMS']} SET title='$title', parent='$parent', description='$description', thumb='$thumb',  comments='$comments', votes='$votes', visibility='$visibility', alb_password='$password', alb_password_hint='$password_hint',keyword='$keyword' WHERE aid='$aid' AND category='$category' LIMIT 1";
        }
        if (in_array($parent,$SUBALB_LIST)||$parent==$aid) {
            cpg_die(CRITICAL_ERROR, $lang_errors['album_loop'], __FILE__, __LINE__);
        }
        $update = cpg_db_query($query);

        if (!mysql_affected_rows()) cpg_die(INFORMATION, $lang_db_input_php['no_udp_needed'], __FILE__, __LINE__);
        pageheader($lang_db_input_php['alb_updated'], "<META http-equiv=\"refresh\" content=\"1;url=modifyalbnew.php?album=$aid\">");
        msg_box($lang_db_input_php['info'], $lang_db_input_php['alb_updated'], $lang_continue, "modifyalbnew.php?album=$aid");
        pagefooter();
        ob_end_flush();
        exit;
        break;
