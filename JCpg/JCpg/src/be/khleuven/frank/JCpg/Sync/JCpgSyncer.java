...
/**
 * Used for syncing the whole gallery. We sync from top to bottom: cats -> albums -> pictures This is important when executing INSERT queries. This means we want to make new entries in the database which don't yet have a correct id, which is autogenerated by the database. So first we insert a new category, we then fetch the generated id and pass it through to the underlying albums and so on.
