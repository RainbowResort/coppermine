 ...
 * showthumb.php
 *
 * Script to create the thumbnails on the fly and display in the browser.
 *
 * @package cpgNG
...
function makethumbnail($src_file, $newSize, $method)
{
    ...
    if ($imginfo[2] == GIS_GIF && $config->conf['GIF_support'] == 0) {
        header("Content-type: image/gif");
    ...
    switch ($method) {
        ...
        case "gd2" :
            if ($imginfo[2] == GIS_GIF && $config->conf['GIF_support'] == 1)
...
makethumbnail($config->conf['fullpath'] . $_GET['picfile'], $_GET['size'], $config->conf['thumb_method']);
?>