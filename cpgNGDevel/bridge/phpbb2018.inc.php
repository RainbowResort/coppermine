...
/*************************
  ...
  ********************************************
  ...
  $Revision$
  $Author: amit $
  $Date: 2006-06-06 15:39:36 +0530 (Tue, 06 Jun 2006) $
**********************************************/

if (!defined('IN_COPPERMINE')) die('Not in Coppermine...');

if (!class_exists('phpbb2018_udb')) { //protects the class from being included more than once.
	...
	class phpbb2018_udb extends core_udb {
        ...
        public function defaults()
        {
            ...
            $defaults->full_name = 'phpBB version 2.0.18 or better';
            $defaults->short_name = 'phpbb2018';
            $defaults->support_url = 'http://www.phpbb.com/';
            $defaults->config->full_forum_url->setting = 'http://www.yoursite.com/board';
            $defaults->config->full_forum_url->used = 'mandatory,not_empty,no_trailing_slash';
            $defaults->config->relative_path_to_config_file->setting = '../board/';
            $defaults->config->relative_path_to_config_file->used = 'lookfor,config.php';
            $defaults->config->use_post_based_groups->setting = '0';
            $defaults->config->use_post_based_groups->used = 'radio,1,0';
            $defaults->config->cookie_prefix->setting = 'phpbb2mysql';
            $defaults->config->cookie_prefix->used = 'cookie';    
            return $defaults;
        }
	    ...
	    function collect_groups()
	    {
	        ...
	        {
	            $udb_groups[$row[$this->field['grouptbl_group_id']]+100] = cpgUtils::utf_ucfirst(cpgUtils::utf_strtolower($row[$this->field['grouptbl_group_name']]));
	        }
	...
	*/
	    ...
	    function get_groups($row)
	    {
	        ...
	        if ($this->use_post_based_groups){
	            ...
	            while ($row2 = $this->cpgDb->fetchRow()) {
	                $data[] = $row2['group_id'];
	            }
	...
	*/
	            $this->cpgDb->query($sql);
	            list($id, $pass, $this->userlevel) = $this->cpgDb->fetchRow();
	            $this->sid = 0;
	        }
	    ...
	    function get_users($options = array())
	    {
	        ...
	        if (in_array($options['sort'], array('pic_a', 'pic_d', 'disku_a', 'disku_d'))){
	            ...
	            ////if (!mysql_num_rows($result)) {
	            if ($this->cpgDb->numRows() <= 0) {
	                return array();
	            }
	
	            ////while ($row = mysql_fetch_assoc($result)) $userlist[$row['owner_id']] = $row;
	            while ($row = $this->cpgDb->fetchRow()) $userlist[$row['owner_id']] = $row;
	            ...
	            while ($row = $this->cpgDb->fetchRow()) $userlist[$row['user_id']] = array_merge($userlist[$row['user_id']], $row);
	            ////mysql_free_result($result);
	            ...
	            while ($row = $this->cpgDb->fetchRow()) $userlist[$row['user_id']] = $row;
	            ////mysql_free_result($result);
	            ...
	            while ($owner = $this->cpgDb->fetchRow()) $userlist[$owner['owner_id']] = array_merge($userlist[$owner['owner_id']], $owner);
	        }
