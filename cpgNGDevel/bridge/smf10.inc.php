...
/*************************
  ...
  ********************************************
  ...
  $Revision$
  $Author: amit $
  $Date: 2006-06-05 11:14:13 +0530 (Mon, 05 Jun 2006) $
**********************************************/
...
if (!class_exists('smf10_udb')) { //protects the class from being included more than once.

	class smf10_udb extends core_udb {
            function defaults()
            {
                $defaults = new stdclass;
                $defaults->full_name = 'Simple Machines (SMF)';
                $defaults->short_name = 'smf10';
                $defaults->support_url = 'http://www.simplemachines.org/';
                $defaults->config->full_forum_url_default = 'http://www.yoursite.com/board';
                $defaults->config->full_forum_url_used = 'mandatory,not_empty,no_trailing_slash';
                $defaults->config->relative_path_to_config_file_default = '../board/';
                $defaults->config->relative_path_to_config_file_used = 'lookfor,Settings.php';
                $defaults->config->use_post_based_groups_default = '0';
                $defaults->config->use_post_based_groups_used = 'radio,1,0'; 
                return $defaults;               
            }
	
	        function smf10_udb()
	        {
	                global $BRIDGE, $CONFIG, $user_settings;
	        ...
	        function authenticate()
	        {
	                ...
	                $USER_DATA = array_merge($USER_DATA, $this->cpgGetUserData($USER_DATA['groups'][0], $USER_DATA['groups'], $this->guestgroup));
