<?php
/**
 * cpgMostViewedAlbumData.class.php
 *
 * Meta album class for showing the most viewed photos
 *
 * @package cpgNG
 ...
 * @access public
 */
class cpgMostViewedAlbumData extends cpgAlbumData {
    var $cpgAlbumName;
    var $db;
    var $config;
    var $auth;
    ...
    /**
     * cpgMostViewedAlbumData::cpgMostViewedAlbumData()
     *
     * @return
     */
    function cpgMostViewedAlbumData()
    {
    ...
    /**
     * cpgMostViewedAlbumData::getThumbnailData()
     *
     * @param  $album
     * @param  $cat
     * @param  $page
    ...
    function getThumbnailData($album, $cat, $page, $thumbcols, $thumbrows, $display_tabs, $indexPage = false)
    {
        ...
        if (count($pic_data) > 0) {
            foreach ($pic_data as $key => $row) {
                ...
                $thumb_list[$i]['displayURL'] = $this->config->conf['ecards_more_pic_target'] . 'displayimage.php?meta=topn';
                ...
                if ($this->config->conf['display_mini_toolbar'] == 1) {
                    ...
                    if ($this->auth->isDefined('USER_CAN_SEND_ECARDS')) {
                        $thumb_list[$i]['ecardURL'] = 'ecard.php?meta=topn&amp;';
            ...
            if ($total_pages > 1) {
                for ($i = 1; $i <= $total_pages; $i++) {
                    $thumb_list['pageURL'][$i] = $this->config->conf['ecards_more_pic_target'] . 'thumbnails.php?meta=topn&amp;page=' . $i;
                    if ($cat) {
    ...
    function getPicData($album, &$count, &$album_name, $limit1 = -1, $limit2 = -1, $cat, $indexPage, $set_caption = true)
    {
        ...
        if (!$indexPage) {
            $query = "SELECT COUNT(pid) AS totalPic FROM {$this->config->conf['TABLE_PICTURES']} WHERE approved = 'YES' AND hits > 0  $albStr $keyword";
            $this->db->query($query);
            $row = $this->db->fetchRow();
            $count = $row['totalPic'];
            unset($row);
