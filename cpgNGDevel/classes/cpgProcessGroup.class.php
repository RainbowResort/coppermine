    ...
    */
    ...
    while ($row = $this->db->fetchRow()) {
      ...
      if ($row['group_id'] == 3 && $this->config->conf['allow_unlogged_access'] == 0) {
        ...
        $this->groupArr[$i]['grayoutHelp'] ='&nbsp;'.cpgUtils::cpgDisplayHelp('f=index.htm&base=64&h='.urlencode(base64_encode(serialize($lang_groupmgr_php['explain_greyed_out_title']))).'&t='.urlencode(base64_encode(serialize(sprintf($lang_groupmgr_php['explain_guests_greyed_out_text'],'<i>'.$group['group_name'].'</i>')))), '450', '300');
      } elseif ($row['group_id'] == 4) {
        $this->groupArr[$i]['disabled'] = 'disabled="disabled" style="background-color:InactiveCaptionText;color:GrayText"';
        $this->groupArr[$i]['grayoutHelp'] = '&nbsp;'.cpgUtils::cpgDisplayHelp('f=index.htm&base=64&h='.urlencode(base64_encode(serialize($lang_groupmgr_php['explain_greyed_out_title']))).'&t='.urlencode(base64_encode(serialize(sprintf($lang_groupmgr_php['explain_banned_greyed_out_text'],'<i>'.$group['group_name'].'</i>')))), '450', '300');
      } else {
      ...
      if (UDB_INTEGRATION == 'coppermine' && isset($defaultGroupNames[$row['group_id']])) {
        if ($row['group_name'] != $defaultGroupNames[$row['group_id']] && $defaultGroupNames[$row['group_id']] != '') {
          ...
          $this->groupArr[$i]['showReset'] = array('title' => sprintf($lang_groupmgr_php['reset_to_default'], $defaultGroupNames[$row['group_id']]), 'onclick' => 'document.groupmanager.group_name_'.$row['group_id'].'.value=\''.$defaultGroupNames[$group['group_id']].'\'');
        }
      }
