<?php
/**
 * cpgMyFavoriteAlbumData.class.php
 *
 * Meta album class for showing the user's favorite pictures
 *
 * @package cpgNG
 ...
 * @access public
 */
class cpgMyFavoriteAlbumData extends cpgAlbumData {
    var $cpgAlbumName;
    var $db;
    var $config;
    var $auth;
    ...
    /**
     * cpgMyFavoriteAlbumData::cpgMyFavoriteAlbumData()
     *
     * @return
     */
    function cpgMyFavoriteAlbumData()
    {
    ...
    /**
     * cpgMyFavoriteAlbumData::getThumbnailData()
     *
     * @param  $album
     * @param  $cat
     * @param  $page
    ...
    function getThumbnailData($album, $cat, $page, $thumbcols, $thumbrows, $display_tabs, $indexPage = false)
    {
        ...
        if (count($pic_data) > 0) {
            foreach ($pic_data as $key => $row) {
                ...
                $thumb_list[$i]['displayURL'] = $this->config->conf['ecards_more_pic_target'] . 'displayimage.php?meta=favpics';
                ...
                // image caption
                $thumb_list[$i]['msgBody'] = strlen($row['caption']) > 50 ? @substr($row['caption'], 0, 50) . "...": $row['caption'];
                ...
                if ($this->config->conf['display_mini_toolbar'] == 1) {
                    ...
                    if ($this->auth->isDefined('USER_CAN_SEND_ECARDS')) {
                        $thumb_list[$i]['ecardURL'] = 'ecard.php?meta=favpics&amp;';
            ...
            if ($total_pages > 1) {
                for ($i = 1; $i <= $total_pages; $i++) {
                    $thumb_list['pageURL'][$i] = $this->config->conf['ecards_more_pic_target'] . 'thumbnails.php?meta=favpics&amp;page=' . $i;
                    if ($cat) {
     ...
     */
    /**
     * cpgMyFavoriteAlbumData::getPicData()
     *
     * @param  $album
     * @param  $count
     * @param  $album_name
    ...
    function getPicData($album, &$count, &$album_name, $limit1 = -1, $limit2 = -1, $cat, $indexPage, $set_caption = true)
    {
        ...
        /**
         * select favorites picture details, if $FAVPICS has values.
         */
        if (!empty($this->auth->favPics)) {
            // my favorite pictures
            $favpics = ' AND pid IN (' . implode (',', $this->auth->favPics) . ')';

            // query to fetch the total no. of thumnails
            if (!$indexPage) {
                $query = "SELECT COUNT(pid) AS totalPic from {$this->config->conf['TABLE_PICTURES']} WHERE approved = 'YES' $favpics $albStr";
                $this->db->query($query);
                $row   = $this->db->fetchRow();
                $count = $row['totalPic'];
                unset($row);
            }
            $query = "SELECT $select_columns FROM {$this->config->conf['TABLE_PICTURES']} WHERE approved = 'YES' $favpics $albStr ORDER BY pid DESC $limit";
            $this->db->query($query);
