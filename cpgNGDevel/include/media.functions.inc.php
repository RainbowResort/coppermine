...
/*************************
  ...
  ********************************************
  ...
  $Source: /home/cvs/cpgNGDevel/include/media.functions.inc.php,v $
  $Revision$
  $Author$
  $Date$
**********************************************/
...
$config->conf['allowed_file_extensions'] = '';
if (count($FILE_TYPES)==0) {
         $query = 'SELECT extension, mime, content FROM '.$config->conf['TABLE_FILETYPES'];
         $db->query($query);
         while ($row = $db->fetchRow()) {
             // Only add types that are in both the database and user defined parameter
             if ($config->conf[$content_types_to_vars[$row['content']]]=='ALL' || is_int(strpos('/'.$config->conf[$content_types_to_vars[$row['content']]].'/','/'.$row['extension'].'/'))) {
                 $FILE_TYPES[$row['extension']] = $row;
                 $config->conf['allowed_file_extensions'].= '/'.$row['extension'];
              }   
         }
}

$config->conf['allowed_file_extensions'] = substr($config->conf['allowed_file_extensions'],1);
