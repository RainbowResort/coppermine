...
class coppermine_udb extends core_udb
{
    ...
    function login($username = null, $password = null, $remember = false)
    {
        ...
        $sql =  "SELECT user_id, user_name, user_password FROM {$this->usertable} WHERE BINARY user_password = '$encpassword' AND user_active = 'YES' AND ";
        
        // Check the login method (username, email address or both)
        switch ($CONFIG['login_method']) {
        
        case 'both':
            $sql .= "(user_name = '$username' OR user_email = '$username')";
            break;
            
        case 'email':
            $sql .= "user_email = '$username'";
            break;
    ...
    function create_session()
    {
        ...
        $sql  = "INSERT INTO {$this->sessionstable} (session_id, user_id, time, remember) ";
        $sql .= "VALUES ('" . md5($session_id) . "', 0, UNIX_TIMESTAMP(), 0)";
    ...
    function generateId()
    {
        ...
        while ($failsafe--) {
            ...
            if ($randnum != "") {
                ...
                $sql = "SELECT null FROM {$this->sessionstable} WHERE session_id = '" . md5($session_id) . "'";
                $result = cpg_db_query($sql, $this->link_id);
        ...
        */
        ...
        foreach ($udb_groups as $i_group_id => $i_group_name) {
            if ($cpg_groups[$i_group_id] != $i_group_name) {
                cpg_db_query("UPDATE {$CONFIG['TABLE_USERGROUPS']} SET group_name = '" . addslashes($i_group_name) . "' WHERE group_id = $i_group_id");
            }
        }
        
        // fix admin grp
        if (!$this->use_post_based_groups) {
            cpg_db_query("UPDATE {$CONFIG['TABLE_USERGROUPS']} SET has_admin_access = 1 WHERE group_id = 1");
        }
    }
}
