...
jQuery(document).ready(function() {
   ...
   // We need not execute the filmstrip js if there are not enough pictures in the album
    ...
    for(var i=0; i<maxItems; i++) {
        url_cache[cacheIndex+i] = jQuery("img.strip_image").eq(i).attr("src");
        link_cache[cacheIndex+i]= jQuery("a.thumbLink").eq(i).attr("href");
        alt_cache[cacheIndex+i] = jQuery("img.strip_image").eq(i).attr("alt");
        title_cache[cacheIndex+i] = jQuery("img.strip_image").eq(i).attr("title");
    }

    // button HTML
    jQuery('td.prev_strip').html("<a id=\"filmstrip_prev\" rel=\"nofollow\" style=\"cursor: pointer;\"><img src=\"./images/icons/"+leftimage+"\" border=\"0\" /></a>");
    jQuery('td.next_strip').html("<a id=\"filmstrip_next\" rel=\"nofollow\" style=\"cursor: pointer;\"><img src=\"./images/icons/"+rightimage+"\" border=\"0\" /></a>");
    
    // hide buttons if no further pics in direction
    if (nextPosition < (NumberOfPics - picQueue)) { jQuery('#filmstrip_next').css( "visibility", "visible"); } else { jQuery('#filmstrip_next').css( "visibility", "hidden"); }
    if (nextPosition > (picQueue-1)) { jQuery('#filmstrip_prev').css( "visibility", "visible"); } else { jQuery('#filmstrip_prev').css( "visibility", "hidden"); }
    ...
    if(nextPosition > (NumberOfPics-picQueue)) {
      ...
      if (thumb_mode != 'wd' && striptype == 'hori') {
        ...
        jQuery('.tape').css("height", stripheight);
      }
      if (striptype == 'vert')
      {
        jQuery('#film').css("height", maxItems*parseInt(width));
        jQuery('#film').css("overflowy" , "hidden");
      }
          
      // Bind an onclick event on element with id filmstrip_next
      jQuery('#filmstrip_next').click(function() {
          ...
          if(((NumberOfPics-1)-(picQueue-1)) <= nextPosition ) {
              jQuery('#filmstrip_next').css( "visibility", "hidden");
          }
          ...
          if(nextPosition > (picQueue-1)) {
              jQuery('#filmstrip_prev').css( "visibility", "visible"); // = "visible";
          }
          ...
          if (!url_cache[nextPosition + $go_next]) {
              ...
              jQuery.getJSON(next_url, function(data) {
                  ...
                  var itemLength = (striptype == 'hori') ? jQuery(".tape tr > .thumb").length : jQuery(".thumb").length;
                  var itemsToRemove = maxItems+1;
                  if (itemLength == itemsToRemove) {
                      if (striptype == 'hori') jQuery('.remove').remove();
                      else jQuery('.remove').parents('tr:eq(0)').remove();
                  }
                  if (striptype == 'hori')
                  {
                    jQuery('.tape').css("marginLeft", '0px');
                  ...
                  else
                  {
                    ...
                    jQuery('.tape').append(thumb);
                    tempWidth = parseInt(width) +10;
                    jQuery('.tape').animate({
                        marginTop: "-"+tempWidth+"px"
                        },250,"linear",function() {stripAniInProgress = 0;} );
                  }
                  jQuery('.thumb').eq(0).addClass("remove");
              });
          } else {
              if (striptype == 'hori')
              {
                var itemLength = jQuery(".tape tr > .thumb").length;
                ...
                {
                  var itemLength = jQuery(".thumb").length;
                  if (itemLength == (maxItems+1)) {
                    jQuery('.remove').parents('tr:eq(0)').remove();
                  }
                  jQuery('.tape').css("marginTop", '0px');
      ...
      jQuery('#filmstrip_prev').click(function() {
          ...
          if(nextPosition_to <= (NumberOfPics-(picQueue))) {
              jQuery('#filmstrip_next').css( "visibility", "visible"); //style.visibility = "visible"; // show();
          }
  
          if(nextPosition_to < (picQueue)) {
              jQuery('#filmstrip_prev').css( "visibility", "hidden");  // hide();
          }
          ...
          if(!url_cache[nextPosition-$go_next]) {
              ...
              jQuery.getJSON(prev_url, function(data) {
                  url_cache[nextPosition-$go_next]  = data['url'];
                  link_cache[nextPosition-$go_next] = data['target'];
                  alt_cache[nextPosition-$go_next] = data['alt'];
                  title_cache[nextPosition-$go_next] = data['title'];
                  ...
                  if (striptype == 'hori')
                  {
                    ...
                    jQuery('.tape').css("marginLeft", '-'+width+'px');
                    ...
                    jQuery('.tape tr').prepend(thumb_prev);
                  ...
                  else
                  {
                    ...
                    jQuery('.tape').css("marginTop", '-'+width+'px');
                    ...
                    jQuery('.tape').prepend(thumb_prev);
                  ...
                  jQuery('.thumb').eq((maxItems)).addClass("remove");
              });
          } else {
              if (striptype == 'hori')
              {
...
*/
function rate(obj) {
    jQuery.get('ratepic.php?rate=' + obj.title + '&pic=' + js_vars.picture_id + '&form_token=' + js_vars.form_token
        ...
        if(json_data.status == 'success') {
            ...
            jQuery('#voting_title').html( json_data.new_rating_text );
        }
        displayStars();
    });
}
...
function changeover(obj) {
    var id = obj.title;
    for(i=0; i<id; i++) {
        jQuery('#' + js_vars.picture_id + '_' + (i+1)).attr('src', js_vars.theme_dir + 'images/rate_new.png');
    }
}
...
function changeout(obj) {
    ...
    for(i=0; i<id; i++) {
        ...
        jQuery('#' + js_vars.picture_id + '_' + (i+1)).attr('src', img);
    }
}

function displayStars() {
    if(js_vars.stars_amount != 'fallback') {
        jQuery('#star_rating').empty();
        ...
        jQuery('#star_rating').append(center);
...
function printSlideshowButton() {
    ...
    jQuery('#slideshow_button').append(btn);
}
...
function printPicInfoButton() {
    ...
    jQuery('#pic_info_button').append(btn);
}
