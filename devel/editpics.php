...
function form_pic_info($text)
{
    ...
    if (UPLOAD_APPROVAL_MODE) {
        $pic_info = $CURRENT_PIC['pwidth'] . 'x' . $CURRENT_PIC['pheight'] . ' - ' . ($CURRENT_PIC['filesize'] >> 10) . $lang_byte_units[1];
...
if (UPLOAD_APPROVAL_MODE) {
    ...
    $sql = "SELECT * " . "FROM {$CONFIG['TABLE_PICTURES']} " . "WHERE approved = 'NO' " . "ORDER BY pid " . "LIMIT $start, $count";
    $result = db_query($sql);
