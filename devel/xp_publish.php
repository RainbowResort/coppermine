...
function simple_die($msg_code, $msg_text,  $error_file, $error_line, $output_buffer = false)
{
	...
	if(!$CONFIG['debug_mode']){
		$msg .=	'(' . $lang_cpg_die['file'] . ': ' . $error_file .
				' / ' . $lang_cpg_die['line'] . ': ' . $error_line . ')';
	}
...
function process_login()
{
	...
	if (mysql_num_rows($results)) {
		...
		$params = array(
			'{WELCOME}' => sprintf($lang_login_php['welcome'], USER_NAME),
			'{POST_ACTION}' => $PHP_SELF.'?cmd=publish',
		);
		...
		$params = array(
			'{ERROR}' => $lang_login_php['err_login'],
			'{POST_ACTION}' => $PHP_SELF.'?cmd=publish',
		);
...
function form_publish()
{
	...
	if (!(USER_CAN_CREATE_ALBUMS || USER_IS_ADMIN)) {
		...
		$params = array(
			'{WELCOME}' => sprintf($lang_xp_publish_php['welcome'], USER_NAME),
...
function create_album()
{
	...
	$params = array(
		'{NEW_ALB_CREATED}' => sprintf($lang_xp_publish_php['new_alb_created'], $HTTP_POST_VARS['new_alb_name']),
