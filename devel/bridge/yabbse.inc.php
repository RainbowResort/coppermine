...
function udb_get_user_name($uid)
{
	...
	$sql = "SELECT realName as user_name ".
		   "FROM ".$UDB_DB_NAME_PREFIX.YS_TABLE_PREFIX.YS_USER_TABLE." ".
		   "WHERE ID_MEMBER = '$uid'";
...
function udb_get_user_infos($uid)
{
	...
	$sql = "SELECT realName as user_name, memberGroup as group_name, emailAddress as user_email, dateRegistered as user_regdate, ".
		   "websiteURL as user_website ".
		   "FROM ".$UDB_DB_NAME_PREFIX.YS_TABLE_PREFIX.YS_USER_TABLE." ".
		   "WHERE ID_MEMBER = '$uid'";
	$result = db_query($sql, $UDB_DB_LINK_ID);
	...
	if ($user_data['group_name'] == 'Administrator') {
		...
		$sql =  "SELECT ID_GROUP as mgroup ".
				"FROM ".$UDB_DB_NAME_PREFIX.YS_TABLE_PREFIX.YS_GROUP_TABLE." ".
				"WHERE membergroup = '{$user_data['group_name']}' ";
		$result = db_query($sql);
...
function udb_list_users_retrieve_data($result, $lower_limit, $count)
{
	...
	$sql = "SELECT ID_MEMBER as user_id, realName as user_name ".
		   "FROM ".$UDB_DB_NAME_PREFIX.YS_TABLE_PREFIX.YS_USER_TABLE." ".
		   "WHERE ID_MEMBER IN $user_id_set";
	$result = db_query($sql, $UDB_DB_LINK_ID);
...
function udb_get_admin_album_list()
{
	...
	if (UDB_CAN_JOIN_TABLES) {
		$sql = "SELECT aid, CONCAT('(', realName, ') ', a.title) AS title ".
			   "FROM {$CONFIG['TABLE_ALBUMS']} AS a ".
			   "INNER JOIN ".$UDB_DB_NAME_PREFIX.YS_TABLE_PREFIX.YS_USER_TABLE." AS u ON category = (".FIRST_USER_CAT." + ID_MEMBER) ".
			   "ORDER BY title";
		return $sql;
	} else {
