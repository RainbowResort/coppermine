...
if ($use_bridgemgr == 0) { // the vars that are used when bridgemgr is disabled
    ...
    define('VB_CUST_NO', $BRIDGE['license_number']); // Your vBulletin license number (NOT your customer number)
    ...
    define('VB_COOKIE_PREFIX', $BRIDGE['cookie_prefix']);  // Cookie Prefix, not supported by vBulletin 2
}
...
function udb_authenticate()
{
    ...
    if ($got_user) {
        ...
        $result = cpg_db_query("SELECT * FROM {$CONFIG['TABLE_USERGROUPS']} WHERE group_id = " . VB_GUEST_GROUP);
        if (!mysql_num_rows($result)) {
...
function udb_synchronize_groups()
{
    ...
    $result = cpg_db_query("SELECT usergroupid, title FROM " . $UDB_DB_NAME_PREFIX . VB_TABLE_PREFIX . VB_GROUP_TABLE . " WHERE 1", $UDB_DB_LINK_ID);
    while ($row = mysql_fetch_array($result)) {
...
function udb_util_filloptions()
{
    ...
    if (UDB_CAN_JOIN_TABLES) {
        ...
        $query = "SELECT aid, category, IF(username IS NOT NULL, CONCAT('(', username, ') ', a.title), CONCAT(' - ', a.title)) AS title " . "FROM {$CONFIG['TABLE_ALBUMS']} AS a " . "LEFT JOIN $usertbl AS u ON category = (" . FIRST_USER_CAT . " + userid) " . "ORDER BY category, title";
        ...
        $user_album_ids_and_names = cpg_db_query("SELECT (userid + ".FIRST_USER_CAT.") as id, CONCAT('(', username, ') ') as name FROM $usertbl ORDER BY name ASC",$UDB_DB_LINK_ID);
