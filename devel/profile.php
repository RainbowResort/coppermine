...
if (isset($HTTP_POST_VARS['change_profile']) && USER_ID && !defined('UDB_INTEGRATION')) {
	...
	$sql = "UPDATE {$CONFIG['TABLE_USERS']} SET ".
...
if (isset($HTTP_POST_VARS['change_password']) && USER_ID && !defined('UDB_INTEGRATION')) {
	...
	$sql = "UPDATE {$CONFIG['TABLE_USERS']} SET ".
		   ...
		   "WHERE user_id = '".USER_ID."' AND BINARY user_password = '$current_pass'";
...
// ------------------------------------------------------------------------- //
	...
	$sql =  "SELECT user_name, user_email, user_group, UNIX_TIMESTAMP(user_regdate) as user_regdate, group_name, ".
			"user_location, user_interests, user_website, user_occupation, user_lang as user_group_list, ".
	...
	if($user_data['user_group_list'] != ''){
		$sql =  "SELECT group_name ".
				...
				"WHERE group_id IN ({$user_data['user_group_list']}) AND group_id != {$user_data['user_group']} ".
				"ORDER BY group_name";
		$result = db_query($sql);
	...
	$form_data = array(
		...
		'disk_usage' => $user_data['disk_usage'].
						($user_data['group_quota'] ? '/'.$user_data['group_quota'] : '').
						' '.$lang_byte_units[1],
...
// ------------------------------------------------------------------------- //
	...
	if(defined('UDB_INTEGRATION')){
		...
		$sql =  "SELECT user_name, user_email, UNIX_TIMESTAMP(user_regdate) as user_regdate, group_name, ".
				"user_location, user_interests, user_website, user_occupation ".
				"FROM {$CONFIG['TABLE_USERS']} AS u ".
				"INNER JOIN {$CONFIG['TABLE_USERGROUPS']} AS g ON user_group = group_id ".
				"WHERE user_id ='$uid'";
	...
	if ($comment_count) {
		$sql =  "SELECT filepath, filename, url_prefix, pwidth, pheight, msg_author, UNIX_TIMESTAMP(msg_date) as msg_date, msg_body ".
				...
				"WHERE msg_id='".$lastcom_id."' AND c.pid = p.pid";
		$result = db_query($sql);
